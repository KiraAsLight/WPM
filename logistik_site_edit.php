...
// Get item data
$item = fetchOne('SELECT * FROM logistik_site WHERE id = ? AND pon = ?', [$itemId, $ponCode]);
if (!$item) {
    header('Location: logistik_site.php?pon=' . urlencode($ponCode) . '&error=not_found');
    exit;
}
...
$old = [
    ...
    'qty' => $item['qty'],
    'sent_to_site' => $item['sent_to_site'],
    'no_truk' => $item['no_truk'],
    'foto' => $item['foto'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $old['sent_to_site'] = (float)($_POST['sent_to_site'] ?? 0);
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($old['foto'] && file_exists(__DIR__ . '/' . $old['foto'])) {
    ...
    $existing = fetchOne(
        'SELECT id FROM logistik_site WHERE pon = ? AND no = ? AND id != ?',
    ...
    if (empty($errors)) {
        try {
            update('logistik_site', $old, 'id = :id', ['id' => $itemId]);

            header('Location: logistik_site.php?pon=' . urlencode($ponCode) . '&updated=1');
            exit;
        } catch (Exception $e) {
...
<head>
    ...
    <title>Edit Item Site - <?= h($appName) ?></title>
...
<body>
    <div class="layout">
        ...
        <header class="header">
            <div class="title">Edit Item Site</div>
            <div class="meta">
        ...
        <main class="content">
            <div class="form-container">
                <div class="form-header">
                    <div class="form-title">Edit Item Site</div>
                </div>
                ...
                <form method="post" class="form" enctype="multipart/form-data">
                    ...
                    <div class="form-row">
                        <div class="field">
                            <label class="label" for="sent_to_site">Sent to Site (kg)</label>
                            <input class="input" type="number" id="sent_to_site" name="sent_to_site" value="<?= h($old['sent_to_site']) ?>" min="0" step="0.01">
                        </div>
                    ...
                    <div class="field full-width">
                        ...
                        <?php if ($old['foto']): ?>
                            <div class="current-foto">
                                <p style="font-size: 12px; color: var(--muted); margin-bottom: 8px;">Foto Saat Ini:</p>
                                <img src="<?= h($old['foto']) ?>" alt="Current Foto">
                            </div>
                        <?php endif; ?>
                        ...
                        <?php endif; ?>
                        <div class="help-text">Kosongkan jika tidak ingin mengubah foto. Format: JPG, PNG, GIF (Max: 5MB)</div>
                    ...
                    <div class="field full-width">
                        ...
                        <textarea class="textarea" name="remarks" id="remarks"><?= h($old['remarks']) ?>"></textarea>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="field">
                            ...
                            <div class="help-text">Progress akan otomatis 100% jika status diubah menjadi "Diterima"</div>
                        </div>
        ...
        <footer class="footer">© <?= date('Y') ?> <?= h($appName) ?> • Edit Site</footer>
    </div>
    ...
    <script>
        ...
        function handleStatusChange(status) {
            if (status === 'Diterima') {
                // Auto-set progress to 100% when status is Diterima
                document.getElementById('progress').value = 100;
                updateProgressPreview(100);
            } else if (status === 'Menunggu') {
                // Reset progress to 0 when status is Menunggu
