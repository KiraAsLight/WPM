...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $valid = ['To Do', 'In Progress', 'Review', 'Blocked', 'Done'];
  if ($idx >= 0 && $idx < count($tasks)) {
    $progress = max(0, min(100, $progress));
    if (!in_array($status, $valid, true))
      $status = (string) ($tasks[$idx]['status'] ?? 'To Do');
...
<body>
            ...
            <div style="overflow:auto">
              <table>
                ...
                <tbody>
                  <?php if (!$rows): ?>
                    <tr>
                      <td colspan="6" class="muted">Belum ada task untuk divisi ini.</td>
                    </tr>
                  <?php else:
                    foreach ($rows as $r):
                      $idx = (int) $r['_idx']; ?>
                        ...
                        <td>
                          <form method="post" class="inline-form"
                            ...
                            <select class="select sm" name="status">
                              <?php foreach (['To Do', 'In Progress', 'Review', 'Blocked', 'Done'] as $st): ?>
  ...
  <script>
    (function () {
      ...
      if (!el) return; const tz = 'Asia/Jakarta';
